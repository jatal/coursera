...
#...This approach was learned from: http://stackoverflow.com/questions/11774703/adding-text-to-a-grid-table-plot
pretty_table <- function(d, title="", footnote="") {
  library(gridExtra)
  table <- tableGrob(d)
  ...
  h <- grobHeight(table)
  w <- grobWidth(table)
  title <- textGrob(title, y=unit(0.5,"npc") + 0.5*h, vjust=0, gp=gpar(fontsize=20))
...
d <-  data.frame(
  "Event Type"=results$top_10_total_health_impact_name, 
  "Total Impact"=results$top_10_total_health_impact_val, 
  "Percentage Overall"=results$top_10_pct_total_health_impact_val
  )
pretty_table(
  d, 
  "Top 10 Weather Event Types by Total Health Impact", 
  "* Total Impact expressed as whole number of people injured or killed"
  )

d <-  data.frame(
  "Event Type"=results$top_10_total_economic_impact_name, 
  "Total Impact"=round(results$top_10_total_economic_impact_val/(1000*1000*1000),2), 
  "Percentage Overall"=results$top_10_pct_total_economic_impact_val
  )
pretty_table(
  d, 
  "Top 10 Weather Event Types by Total Economic Impact",
  "* Total Impact expressed in Billions"
  )
